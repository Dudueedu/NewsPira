...
session_start(); 
if (!isset($_SESSION['publisher']) || $_SESSION['publisher'] != 1) {
    ...
    <div class="container-fluid pt-5 mb-3">
        <div class="container">
            <div class="section-title">
                <h4 class="m-0 text-uppercase font-weight-bold">Tarefas</h4>
            </div>
            <div class="owl-carousel news-carousel carousel-item-4 position-relative">
                <?php
                    require_once 'conecta.php'; 
                    $consulta = "SELECT imagem FROM filmes WHERE stat = 1  ORDER BY DATACADASTRO DESC LIMIT 1";
                    ...
                    while($dado = $con->fetch_array()){
                            ...
                            echo '<a class="h6 m-0 text-white text-uppercase font-weight-semi-bold" href="./criafilme.php">Filmes</a>';
                            echo "</div>";
                            echo "</div>";
                } ?>
                <?php
                    require_once 'conecta.php'; 
                    $consulta = "SELECT imagem FROM news WHERE stat = 1  ORDER BY DATACADASTRO DESC LIMIT 1";
                    $con = $mysqli->query($consulta) or die ($mysqli->error);
                    while($dado = $con->fetch_array()){
                            echo "<div class='position-relative overflow-hidden' style='height: 300px;'>";
                            echo "<img class='img-fluid h-100' src='".$dado['imagem']."' style='object-fit: cover;''>";
                            echo '<div class="overlay">';
                            echo '<a class="h6 m-0 text-white text-uppercase font-weight-semi-bold" href="./crianot.php">Noticias</a>';
                            echo "</div>";
                            echo "</div>";
                } ?>
                <?php
                    require_once 'conecta.php'; 
                    $consulta = "SELECT imagem FROM classificados WHERE stat = 1  ORDER BY DATACADASTRO DESC LIMIT 1";
                    $con = $mysqli->query($consulta) or die ($mysqli->error);
                    while($dado = $con->fetch_array()){
                            echo "<div class='position-relative overflow-hidden' style='height: 300px;'>";
                            echo "<img class='img-fluid h-100' src='".$dado['imagem']."' style='object-fit: cover;''>";
                            echo '<div class="overlay">';
                            echo '<a class="h6 m-0 text-white text-uppercase font-weight-semi-bold" href="./criaemprego.php">Classificados</a>';
                            echo "</div>";
                            echo "</div>";
                } ?>
                
                <div class='position-relative overflow-hidden' style='height: 300px;'>
                <img class='img-fluid h-100' src="./imagens_banco/admin.jpg" style='object-fit: cover;'>
                <div class="overlay">
                <a class="h6 m-0 text-white text-uppercase font-weight-semi-bold" href="./admin.php">Admin</a>
                </div>
                </div>
            </div>
        </div>
